...
このフォークでは、フォーク専用の Sparkle appcast と EdDSA 署名鍵を設定するまで自動アップデートを無効にしています。元プロジェクトのアップデート経路は信頼せず、そこからアップデートをインストールしません。署名設定が完了するまでは、フォーク版の GitHub Releases ページを使用してください。
